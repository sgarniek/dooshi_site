...
exports.handler = async (event) => {
  ...
  const { recipients, coupon } = JSON.parse(event.body || '{}');

  if (!recipients?.length || !coupon) {
  ...
  const minOrderLine = coupon.min_order_amount > 0
    ? `<p style="font-size:0.82rem; color:#7A6A50; margin-bottom:16px;">מינימום הזמנה לשימוש בקופון: ₪${coupon.min_order_amount}</p>`
    : '';

  const emails = recipients.map(({ to, firstName }) => ({
    from:    'Dooshi <noreply@dooshi.co.il>',
    to:      [to],
    subject: `קופון מיוחד עבורך — Dooshi`,
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; color: #1A1208;">
        ...
        <div style="padding: 32px 24px; background: #FDFCF8; border: 1px solid #e2d8c8;">
          ...
          <p style="font-size: 0.9rem; color: #7A6A50; line-height: 1.6; margin-bottom: 24px;">
            הכנו עבורך קופון הנחה מיוחד. השתמש בקוד למטה בעת ביצוע הזמנה.
          </p>
          ...
          <p style="font-size: 0.78rem; color: #7A6A50; line-height: 1.6;">
            הקופון מיועד עבורך אישית ואינו ניתן להעברה.
          </p>
        </div>
  ...
  if (!res.ok) {
    ...
    return { statusCode: 500, headers: CORS_HEADERS, body: 'Failed to send emails' };
  }
